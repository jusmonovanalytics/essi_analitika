import type {
  ...
  ProductAnalyticsData, ProductDashboardFilters,
} from '../types/api'
...
export type DateFilters = {
  dateFrom: string
  dateTo: string
  /** Sana qaysi ustunga tegishli: buyurtma yaratilgan kun yoki yetkazilgan kun. */
  dateField?: 'created_date' | 'date_delivery'
...
export const fetchKpis = (f: DateFilters) =>
  get<KPIData>('/api/kpis', {
    dateFrom: f.dateFrom, dateTo: f.dateTo, dateField: f.dateField,
...
export const fetchFilterOptions = (dateFrom: string, dateTo: string, dateField?: string) =>
  get<FilterOptions>('/api/filters', { dateFrom, dateTo, dateField })
...
export const fetchStatusStats = (dateFrom: string, dateTo: string, dateField?: string) =>
  get<StatusPoint[]>('/api/status-stats', { dateFrom, dateTo, dateField })

export const fetchChartsExtended = (dateFrom: string, dateTo: string, statuses?: string[] | null, dateField?: string) =>
  get<ExtendedChartsData>('/api/charts-extended', { dateFrom, dateTo, status: joinStrs(statuses), dateField })
...
export const fetchProductAnalytics = (f: DateFilters, limit = 500, pf?: ProductDashboardFilters) =>
  get<ProductAnalyticsData>('/api/product-analytics', {
    dateFrom: f.dateFrom, dateTo: f.dateTo, dateField: f.dateField, limit,
    agentId: joinIds(f.agentId), region: joinStrs(f.region),
    paymentType: joinStrs(f.paymentType), deliveryManId: joinIds(f.deliveryManId),
      status: joinStrs(f.statusFilter),
      operator: joinStrs(pf?.operators), deliveryName: joinStrs(pf?.deliveries),
      department: joinStrs(pf?.departments), productId: joinIds(pf?.productIds),
      zone: joinStrs(pf?.zones),
  })
